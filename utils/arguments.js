...
const resolveDirectory = argv => {
	...
	if (dirs.length > 1) {
		throw new Error(
			`expected 1 directory, received ${dirs.length} - ${dirs}`
		);
	}
	return dirs[0]?.trim?.();
};
	...
	constructor(config = ArgumentExtractor.#defaultConfig) {
		if (!config?.flags) {
			throw new ErrorWithCode(
				`no 'flags' key in config`,
				ArgumentExtractor.ERRORS.MISSING_CONFIG
			);
		}

		if (!config?.options) {
			throw new ErrorWithCode(
				`no 'options' key in config`,
				ArgumentExtractor.ERRORS.MISSING_CONFIG
			);
		}
